...
export function validatePalette(palette: Palette): ValidationResult {
  ...
  // Accents used as syntax highlighting: 4.5:1 against base
  // "central" is only used for errors/deletions so 3:1 is sufficient
  ...
  for (const [name, hex] of Object.entries(palette.accents || {})) {
    ...
    check(`${name} on base`, hex, bg.base, minRatio);
